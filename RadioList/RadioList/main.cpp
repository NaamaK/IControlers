...
int main(void) {
	...
	*/
	...
	RadioList l(c, cor, _text, hStdin, hStdout);
...
VOID KeyEventProc(KEY_EVENT_RECORD ker, COORD& _tempCor, MyLine* _text) {
	...
	if (ker.bKeyDown){
		...
		//space bar
			...
			COORD checkCor = _tempCor;

			//setting the normal color back
			SetConsoleTextAttribute(h, c);

			int rowPlace = (_tempCor.Y - FIRST_ROW);
			int i = 0;

			while (i < 2){
				rowPlace = (rowPlace + 1) % 3;
				checkCor = _tempCor;
				checkCor.Y = FIRST_ROW + rowPlace;

				//check if this line is marked
				if (_text[rowPlace].getFlag() == true){
					SetConsoleCursorPosition(h, checkCor);
					cout << _text[rowPlace].getUnMarked();
					_text[rowPlace].changeFlag();
					SetConsoleCursorPosition(h, _tempCor);
				}
